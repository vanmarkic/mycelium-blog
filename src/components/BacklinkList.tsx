...
export default function BacklinkList({ nodeId, connections }: BacklinkListProps) {
  ...
  return (
    <section className="mt-12 pt-8 border-t border-gray-800">
      <h2 className="text-2xl font-bold text-gray-100 mb-2">🍄 Mycelium Links</h2>
      <p className="text-gray-400 mb-6">Connected through shared concepts, tools, and patterns</p>
                ...
                .map((node) => {
                  ...
                  return (
                    <a
                      key={node.id}
                      href={node.path}
                      className="block p-4 bg-gray-800 rounded-lg hover:bg-gray-750 hover:border-mycelium-600 border border-transparent transition-all group"
                    >
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <h4 className="text-mycelium-400 group-hover:text-mycelium-300 transition-colors font-medium">
                            {node.title}
                          </h4>
                          {edge?.reason && (
